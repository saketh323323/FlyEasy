...
def login():
    ...
    if request.method == 'POST':
        ...
        elif not hashlib.sha256(password.encode()).hexdigest()==user['passwd']:
...
def all_users_view():
    res = view('users')
    print(res)
    return render_template('all_users_view.html', result=res)
...
#         input_password=generate_password_hash(_passwd)
#         first_colon_index = input_password.find(':')
       
#         second_colon_index = input_password.find(':', first_colon_index + 1)
        
# # Check if there are at least two colons
#         if second_colon_index != -1:
#     # Slice the string from the position after the second colon
#             result = input_password[second_colon_index + 1:]
#         else:
#     # If there is no second colon, return the original string or handle accordingly
        ...
        if error is None:
            cursor.execute(
                "INSERT INTO users VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')".format(
                    _uname,
                    hashlib.sha256(_passwd.encode()).hexdigest() ,
                    _pno,
                    _fname,
                    _lname,
                    _dob,
...
def print1():
    if request.method == 'GET':
        ...
        print(finalpay)
        return render_template('print.html', payment = finalpay)
